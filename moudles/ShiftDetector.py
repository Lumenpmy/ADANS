...
class ShiftDetector:
    ...
    #     x_hist, _ = np.histogram(x, bins=bins, density=True)
    #     y_hist, _ = np.histogram(y, bins=bins, density=True)
    #     observed_stat = entropy(x_hist + 1e-10, y_hist + 1e-10)  # 加入一个小常数以避免除零
    #     print("KL divergence between old set and new set is:", observed_stat)
    #
    #     def generate_samples(n):
    #         # 生成两个正态分布的随机样本
    #         x = np.random.normal(0, 1, n)
    ...
    #     for i in range(iterations):
    #         x_sample, y_sample = generate_samples(max(n, m))
    #         bins = np.linspace(min(x_sample.min(), y_sample.min()), max(x_sample.max(), y_sample.max()), 100)
    #         x_hist, _ = np.histogram(x_sample, bins=bins, density=True)
    #         y_hist, _ = np.histogram(y_sample, bins=bins, density=True)
    #         simulated_stat = entropy(x_hist + 1e-10, y_hist + 1e-10)  # 加入一个小常数以避免除零
    #         if simulated_stat >= observed_stat:
    #             count += 1
    #
    #     p_value = (count + 1) / (iterations + 1)
    ...
    def visualize_hists1(self,
        ...
        plt.bar(x, res_1[0], width=width, alpha=0.7, ec='black', color=color_1)
